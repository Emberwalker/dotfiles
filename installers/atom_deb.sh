...
if ! hash gdebi 2>/dev/null; then
  echo '!! gdebi is not available; install with `apt install gdebi` then rerun this script.'
  exit -1
fi

## cURL
if ! hash curl 2>/dev/null; then
  echo '!! curl is not available; install with `apt install curl` then rerun this script.'
  exit -1
fi

## jq
if ! hash jq 2>/dev/null; then
  echo '!! jq is not available; install with `apt install jq` then rerun this script.'
  exit -1
fi
